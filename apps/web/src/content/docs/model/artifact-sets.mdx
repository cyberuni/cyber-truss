...
An **artifact** is one type of thing in a repository. Not one file, and not one line: a
type.
...
This axis is **state-driven**. It needs no diff at all (a cold repository can be evaluated
against it), and it asks: *does this conform to the criteria that govern it?*
...
Each artifact-set is held consistent by a [controller](/cyber-truss/model/controller/), on
a spectrum from agent definition to deterministic code.

## Lifting

Git reports a change as lines in files. The model speaks in artifact-sets. Nothing can be
matched until the delta is **lifted**, raised from a line diff into the set vocabulary.
