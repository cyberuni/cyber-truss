...
Guide for developing `cyber-truss` locally. AI coding assistants should also read [AGENTS.md](AGENTS.md).
...
pnpm ct dev --help       # run the CLI from source, no build (tsx)
pnpm ct test:watch       # watch mode
```
...
Pages live in `apps/web/src/content/docs/`. The `cli/` and `concepts/` directories are
autogenerated into the sidebar, so a new file in either appears without editing
`astro.config.mjs`.
...
`scripts/sync-plugin-version.mjs`, which carries the new version into every plugin
manifest — add any new manifest to that script's list.
